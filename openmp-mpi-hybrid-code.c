...
int countMatchingOpenMP(Transaction *data, int size, const char *location, const char *gender) {
    int count = 0;

    #pragma omp parallel for reduction(+:count) num_threads(8)
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        int choice;
        printf("\nEnter location number to filter: "); // on output buffer
        fflush(stdout); //force krnwd show krnna(right now)
        scanf("%d", &choice);
        while (choice < 1 || choice > locationCount) {
            printf("Invalid choice. Enter between 1 and %d: ", locationCount);
            fflush(stdout);
            scanf("%d", &choice);
        }
    ...
    if (rank == 0) {
        for (int i = 1; i < size; i++) {
            ...
            if (send_count > 0) {
                //all data pointer eka offset krnw, ethna idan how many bytesto send, in byter, 0--tag(msg eka identify kraganna)
        ...
        memcpy(local_data, all_data, own_count * sizeof(Transaction)); //all data pointer eken point krgena inna own_copy size byte tika local ekaa t copy krnna
    } else {
        MPI_Status status;
    ...
    int local_result = countMatchingOpenMP(local_data, local_count, location, gender);
    ...
    if (rank == 0) {
        ...
        printf("  Elapsed Time (MPI + OpenMP): %.4f seconds\n", end_time - start_time);
        free(all_data);
    }
